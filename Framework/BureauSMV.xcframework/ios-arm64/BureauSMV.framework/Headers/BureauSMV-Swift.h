...
#ifndef BUREAUSMV_SWIFT_H
#define BUREAUSMV_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="BureauSMV",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
SWIFT_ENUM_FWD_DECL(NSInteger, AuthenticationStatusCode)
@class NSString;
...
/// Objective-C compatible authentication with PSP callback URLs on the Initiate request.
- (void)makeAuthCallWithMobile:(NSString * _Nonnull)mobile correlationId:(NSString * _Nonnull)correlationId pspCallbacks:(NSArray<NSString *> * _Nonnull)pspCallbacks completion:(void (^ _Nonnull)(AuthenticationResult * _Nonnull))completion;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
SWIFT_ENUM_FWD_DECL(NSInteger, Mode)
